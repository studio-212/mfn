...
/* Official platform glyphs (simple-icons, CC0), drawn in currentColor so they
   inherit the link's cream and its opacity states. Keyed by a squashed platform
   name so "Apple Music", "apple music" and "AppleMusic" all resolve. */
...
function platformIcon(name) {
  ...
  return d ? `<svg class="stream-icon" viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path d="${d}"/></svg>` : '';
}
...
async function loadHomeLogos() {
  ...
  try {
    ...
    el.innerHTML = data.map(l => {
      // An explicitly uploaded logo still wins; otherwise use the built-in glyph,
      // and only fall back to the plain name for a platform we have no mark for.
      const inner = l.image_url
        ? `<img class="stream-logo-img" src="${esc(l.image_url)}" alt="${esc(l.platform)}">`
        : (platformIcon(l.platform) || `<span class="stream-link-name">${esc(l.platform)}</span>`);
